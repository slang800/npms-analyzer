...
function untar(file) {
    ...
    return exec(`tar -xf ${file} -C ${destDir} --strip-components=1 | grep -v "unknown extended header"`)
    ...
    .catch((err) => /(unrecognized archive format|does not look like a tar archive)/i.test(err.message), (err) => {
